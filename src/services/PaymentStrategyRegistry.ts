...
export default class PaymentStrategyRegistry {
    ...
    constructor() {
        ...
        this.registerStrategy(REGISTRY_KEY.STRIPE, new StripeService(new Stripe('testStripeKey')));
        this.registerStrategy(REGISTRY_KEY.CASH, new CashAdapterService(new CashPaymentProcessor()));
    }
     ...
     * @param strategy 
     */
    private registerStrategy(gatewayType: string, strategy: PaymentProcessorInterface): void {
     ...
     * @returns 
     */
    public getStrategy(gatewayType: string): PaymentProcessorInterface | undefined {
